...
#define DEVICE_NAME                          "SC5150000000"                               /**< Name of device. Will be included in the advertising data. */
...
#define BLE_UART_MAX_DATA_LEN				(GATT_MTU_SIZE_DEFAULT - 3)  				/**< Maximum length of data (in bytes) that can be transmitted by the Nordic UART service module to the peer. */
#define START_STRING						"Hello Kzy...\n"
 ...
 */
static void temperature_measurement_send()
{
...
/**@brief    Function for handling the data from the App.
 *
 * @details  
 *           
 */
...
uint8_t g_buf[30] = {0x00};

static void hts_rcv_data_handler(ble_hts_t* p_hts, uint8_t* p_data, uint16_t length)
{
	...
	if(length == strlen("START") && !memcmp(p_data, "START", length))
	{
		temperature_measurement_send();				//Collect once temperature
	}
}
...
static void services_init(void)
{
    ...
    hts_init.temp_type                   = BLE_HTS_TEMP_TYPE_MOUTH;
	hts_init.data_handler 				 = hts_rcv_data_handler;

    // Here the sec level for the Health Thermometer Service can be changed/increased.
    BLE_GAP_CONN_SEC_MODE_SET_ENC_NO_MITM(&hts_init.hts_meas_attr_md.cccd_write_perm);		//ÓÖÊÇ¿ÓËÀÁË
...
static void sensor_sim_init(void)
{
    ...
    m_battery_sim_cfg.start_at_max = false;
    ...
    m_temp_celcius_sim_cfg.start_at_max  = true;
...
//        success = nrf6350_lcd_off();
		success = nrf6350_lcd_write_string("Hello Kzy", 9, 0, 0);
